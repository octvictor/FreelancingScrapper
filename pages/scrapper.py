"""Scrapper page: LinkedIn Sales Navigator lead scraping + its settings.

Part of the Freelancing Tools app - see app.py for the navigation shell.
"""
import os
...
mock = st.sidebar.toggle(
    ...
    help="ON: uses realistic fake data - no login, no network, no risk. "
    "OFF: scrapes real LinkedIn using your saved credentials.",
)
...
if mock:
    ...
    st.sidebar.warning(
        "Safe mode is OFF. LinkedIn Sales Navigator will now log into "
        "your real account and drive a real browser session - that's a ToS "
        "violation and carries real risk of account restriction. Keep run "
        "sizes small and prefer your own account only."
    )
...
tab_linkedin, tab_settings = st.tabs(["LinkedIn Sales Navigator", "Settings"])
...
with tab_settings:
    ...
    if saved:
        ...
        st.success("Saved. The LinkedIn tab will pick this up on its next run.")
